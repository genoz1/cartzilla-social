...
function collectionContext(collection, recentTitles = []) {
  const avoidSection = recentTitles.length
    ? `\n\nTopics already covered recently in this category — pick something meaningfully
different from ALL of these, not just a reworded version of one of them:\n${recentTitles.map((t) => `- ${t}`).join('\n')}`
    : '';
...
claim specific facts about any of these): ${collection.sampleProductTitles.join(', ')}${avoidSection}`;
}
...
async function generateArticle(collection, articleType, recentTitles = []) {
  ...
  const raw = await askClaude(system, collectionContext(collection, recentTitles), 2200);
