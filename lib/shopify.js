...
module.exports = { fetchActiveProducts, fetchEligibleCollections };

function publicCollectionUrl(handle) {
  const domain = process.env.CARTZILLA_SHOPIFY_PUBLIC_DOMAIN || process.env.CARTZILLA_SHOPIFY_STORE_DOMAIN;
...
async function shopifyGet(path) {
  const accessToken = await getAccessToken();
  const url = `https://${process.env.CARTZILLA_SHOPIFY_STORE_DOMAIN}/admin/api/${API_VERSION}/${path}`;
...
// Returns collections that have at least 2 currently-eligible products in
// them (custom_collections + smart_collections), each with a couple of
// representative product titles for context — enough for the caption
// generator to know what kind of part the collection covers, without
// needing every product's full description.
async function fetchEligibleCollections() {
  assertConfigured();
  const { isExcludedCollection } = require('../scripts/product-preferences');
  ...
  const collections = [...(customData.custom_collections || []), ...(smartData.smart_collections || [])];

  const eligible = [];
  for (const c of collections) {
    if (!c.handle) continue;
    if (isExcludedCollection(c)) continue; // sales-channel feed, brand filter, etc. — not a real reader-facing category
    const productsData = await shopifyGet(`products.json?collection_id=${c.id}&status=active&limit=10`);
    const products = (productsData.products || [])
      .filter((p) => p.status === 'active' && !!p.published_at)
      .filter(hasStock)
      .filter(hasUsableImage);

    if (products.length < 2) continue; // not enough real products to justify a category post

    eligible.push({
      id: c.id,
      title: c.title,
      url: publicCollectionUrl(c.handle),
      imageUrl: (c.image && c.image.src) || products[0].images[0].src,
      sampleProductTitles: products.slice(0, 4).map((p) => p.title),
    });
  }
  return eligible;
}
