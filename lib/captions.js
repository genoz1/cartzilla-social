...
// Fixed, guaranteed safety lines — appended in code rather than left to the
// AI to decide whether to include, since this matters too much to be
// conditional on model behavior. Which line applies depends on whether the
// category is actually electrical in nature (checked deterministically
// below) — an electrical warning on a post about mirrors or seat cushions
// would read as an obvious mismatch, so this isn't one-size-fits-all.
const ELECTRICAL_DISCLAIMER = 'Always disconnect power before inspecting any electrical component, and consult a qualified technician if you\'re not comfortable doing this yourself.';
const GENERAL_DISCLAIMER = 'If you\'re ever unsure about a repair or installation, it\'s best to consult a qualified technician.';

const ELECTRICAL_KEYWORDS = [
  'battery', 'batteries', 'solenoid', 'controller', 'motor', 'wiring', 'wire',
  'switch', 'charger', 'charging', 'fuse', 'relay', 'voltage', 'electrical',
  'headlight', 'light', 'led', 'horn', 'ignition', 'key switch', 'speed sensor',
  'onboard computer', 'obc', 'converter', 'inverter', 'alternator',
];

function isElectricalTopic(collection) {
  const haystack = [collection.title, ...(collection.sampleProductTitles || [])]
    .join(' ')
    .toLowerCase();
  return ELECTRICAL_KEYWORDS.some((kw) => haystack.includes(kw));
}

async function generateCategoryHowToPost(collection) {
  const electrical = isElectricalTopic(collection);
  const disclaimer = electrical ? ELECTRICAL_DISCLAIMER : GENERAL_DISCLAIMER;

  const system = `${BASE_RULES}

POST TYPE: Category how-to / diagnostic guide.
You are writing a short, genuinely helpful diagnostic or "how to test/check"
guide about a general type of golf cart part — NOT about one specific
product.
...
this general part category. ${electrical
    ? 'Use only well-established, standard, non-hazardous diagnostic steps (visual inspection, listening for clicks, basic multimeter continuity checks, checking connections) — do NOT invent specific voltage numbers, model-specific steps, or anything that requires disassembling high-voltage systems unsafely.'
    : 'This is a mechanical/cosmetic category, not an electrical one — focus on visual/physical inspection steps appropriate to it (cracks, looseness, wear, fit) rather than anything electrical.'
  } A safety disclaimer will be appended automatically after your response, so you do NOT need to write your own — focus on the diagnostic content itself. End by directing the reader to browse the category for a replacement if that's what the diagnosis points to — do not name a specific product, only the category itself.
...
URL — use the collection URL provided for the link.`;
  ...
  // and using whichever disclaimer actually fits the topic.
  captions.facebook = `${captions.facebook}\n\n${disclaimer}`;
  captions.instagram = `${captions.instagram}\n\n${disclaimer}`;
  captions.threads = `${captions.threads}\n\n${disclaimer}`;
  captions.pinterest_description = `${captions.pinterest_description} ${disclaimer}`;
